...
CFG_FILE2="cfgs/waymo_models/dsvt_pillar_whole.yaml"
CFG_FILE3="cfgs/waymo_models/dsvt_voxel_whole.yaml"
...
CUDA_VISIBLE_DEVICES=2,3 torchrun --nproc_per_node=${NGPUS} --rdzv_endpoint=localhost:${PORT} train.py --launcher pytorch ${ARG2}
CUDA_VISIBLE_DEVICES=2,3 torchrun --nproc_per_node=${NGPUS} --rdzv_endpoint=localhost:${PORT} train.py --launcher pytorch ${ARG3}
