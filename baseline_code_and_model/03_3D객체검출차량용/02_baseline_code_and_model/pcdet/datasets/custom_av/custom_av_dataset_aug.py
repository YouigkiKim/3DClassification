...
def resample_to_64_channel(point_features):
    ...
    for i in range(1, 128, 2):
        ...
        selected_points.append(point_features[segment_mask])
...
class CustomAvDataset(DatasetTemplate):
    ...
    def get_label(self, idx):
        label_file = Path(f"/home/ailab/git/Team_3/baseline_code_and_model/03_3D객체검출차량용/02_baseline_code_and_model/labels/{idx}.txt")
    ...
    def get_lidar(self, idx):
        ...
        if idx[2] == '0': # 64 channel
            return point_features
        elif (idx[2] == '1') or (idx[2] == '2'): # 128 channel
            resampled_cloud = resample_to_64_channel(point_features)
    ...
    def create_groundtruth_database(self, info_path=None, used_classes=None, split='train'):
        ...
        database_save_path = Path(self.root_path) / ('Team_3_label2_augmentation_64_gt_database' if split == 'train' else ('Team_3_label2_augmentation_64_gt_database_%s' % split))
        db_info_save_path = Path(self.root_path) / ('Team_3_label_augmentation_64_dbinfos_%s.pkl' % split)
...
def create_custom_av_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    train_split, val_split = 'train_origin3', 'val_origin3'
    num_features = len(dataset_cfg.POINT_FEATURE_ENCODING.src_feature_list)

    train_filename = save_path / ('Team_3_label_augmentation_64_%s.pkl' % train_split)
    val_filename = save_path / ('Team_3_label_augmentation_64_%s.pkl' % val_split)
